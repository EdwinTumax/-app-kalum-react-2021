...
export default function AsignacionAlumno({ asignacionData, registro, user}) {
    ...
    const [asignacionAlumno,setAsignacionAlumno] = useState({fechaAsignacion:asignacionData.fechaAsignacion, 
            alumno: {carne: asignacionData.alumno.carne},clase: {claseId: asignacionData.clase.claseId} }); 

    const history = useHistory();
    const distpach = useDispatch();

    useEffect(()=>{
        axios.get(`${CLASES_ENPOINT}`).then(({data}) => {
            setClases(data);            
        }).catch(error => {
            if(error.sattus === 401){
                distpach(logoutUser);
    ...
    const actualizarAsignacion = async() => {
        try{
            ...
            if(error.status === 401){
                Swal.fire('Actualización asignacion',`Error: ${error.message}`,'error');
                setShow(false);
                distpach(logoutUser());
            }else {
                Swal.fire('Actualización asignacion',`Error: ${error.message}`,'error');
                ...
                <Modal.Body>
                    <Form>
                        <Form.Group control = "carne">
                            ...
                            <Form.Control className='bg bg-light' plaintext readOnly defaultValue={user.carne}/>
                        </Form.Group>
                        <Form.Group control = "apellidos">
                            <Form.Label>Apellidos</Form.Label>
                            <Form.Control className='bg bg-light' plaintext readOnly defaultValue={user.apellidos}/>
                        </Form.Group>
                        <Form.Group control = "nombres">
                            <Form.Label>Nombres</Form.Label>
                            <Form.Control className='bg bg-light' plaintext readOnly defaultValue={user.nombres}/>
                        </Form.Group>                    
                        <Form.Group control = "nombres">
